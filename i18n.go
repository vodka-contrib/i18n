...
import (
	...
	"github.com/vodka-contrib/i18n/libraries/i18n"
...
func I18n(options ...Options) vodka.MiddlewareFunc {
	...
	return func(next vodka.HandlerFunc) vodka.HandlerFunc {
		return func(ctx vodka.Context) error {
			...
			lang := ctx.QueryParam(opt.Parameter)
			...
			if !hasCookie {
				...
				cookie.SetExpires(time.Now().Add(365 * 24 * time.Hour))
			...
			if opt.Redirect && isNeedRedir {
				ctx.Redirect(http.StatusTemporaryRedirect, opt.SubURL+ctx.Request().URI()[:strings.Index(ctx.Request().URI(), "?")])
			}
			return next(ctx)
		}
	}
